...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            if([[self dataSource] respondsToSelector:@selector(calendarView:eventsForDate:)])
            {
                NSArray *temp=[[self dataSource] calendarView:self eventsForDate:workingDate];
                
                BOOL showDot = ([temp count] > 0);
...
- (void)setDisplayMode:(CKCalendarDisplayMode)displayMode animated:(BOOL)animated
{
    ...
    NSDate * firstDate = [self _firstVisibleDateForDisplayMode:displayMode];
    NSDate * lastDate  = [self _lastVisibleDateForDisplayMode:displayMode];
    
    NSMutableArray *eventArray = [NSMutableArray new];
    
    NSDate *theDate =firstDate;

    for (NSInteger i =0; i>-1; i++) {
        
        [eventArray addObjectsFromArray:[self.dataSource calendarView:self eventsForDate:theDate]];
        
        theDate = [theDate dateByAddingTimeInterval:60*60*24*1];
        
        if ( ([theDate compare:lastDate] == NSOrderedDescending)) {
    ...
    if ([[self dataSource] respondsToSelector:@selector(calendarView:eventsForDate:)]) {
        ...
        [[self dataSource] calendarView:self eventsForDate:[self date]];
        
        NSArray *sortedArray = [eventArray sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            NSDate *d1 = [obj1 date];
            NSDate *d2 = [obj2 date];
    ...
    NSInteger newIndex = [[self calendar] daysFromDate:firstDate toDate:[self date]];
