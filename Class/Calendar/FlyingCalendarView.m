...
@interface FlyingCalendarView () <FlyingCalendarHeaderViewDataSource, FlyingCalendarHeaderViewDelegate, UITableViewDataSource, UITableViewDelegate> {
...
@property (nonatomic, strong) FlyingCalendarHeaderView *headerView;
...
@implementation FlyingCalendarView
...
-(void)commonInitializer {
    ...
    _displayMode = FlyingCalendarViewModeMonth;
    ...
    _headerView = [FlyingCalendarHeaderView new];
...
{
    for (FlyingCalendarCell *cell in [self usedCells]) {
...
- (CGRect)_rectForDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
    CGSize cellSize = [self _cellSize];
    
    CGRect rect = [[[UIApplication sharedApplication] keyWindow] bounds];
    
    if(displayMode == FlyingCalendarViewModeDay)
    {
    ...
    if (displayMode == FlyingCalendarViewModeWeek) {
    ...
    else if(displayMode == FlyingCalendarViewModeMonth)
    {
        rect = [self _rectForCellsForDisplayMode:displayMode];
        rect.size.height += [[self headerView] frame].size.height;
        rect.origin.y -= [[self headerView] frame].size.height;
...
- (CGRect)_rectForCellsForDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
    CGSize cellSize = [self _cellSize];
    ...
    else if(displayMode == FlyingCalendarViewModeWeek)
    {
    ...
    {
        CGFloat width = (CGFloat)[self _columnCountForDisplayMode:FlyingCalendarViewModeMonth] * cellSize.width;
        CGFloat height = (CGFloat)[self _rowCountForDisplayMode:FlyingCalendarViewModeMonth] * cellSize.height;
...
- (void)layoutSubviewsAnimated:(BOOL)animated
{
    ...
    FlyingCalendarHeaderView *header = [self headerView];
    ...
    if (self.displayMode == FlyingCalendarViewModeDay)
    {
...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            FlyingCalendarCell *cell = [self _dequeueCell];
            ...
            if (cellRepresentsToday && isThisMonth && isInRange) {
                [cell setState:FlyingCalendarMonthCellStateTodayDeselected];
            }
            else if(!isInRange)
            {
                [cell setOutOfRange];
                ...
                [cell setState:FlyingCalendarMonthCellStateInactive];
            }
            else
            {
                [cell setState:FlyingCalendarMonthCellStateNormal];
            }
...
{
    for (FlyingCalendarCell *cell in cellsBeingAnimatedIntoView) {
        CGRect frame = [cell frame];
        frame.origin.y -= yOffset;
        [cell setFrame:frame];
    }
    for (FlyingCalendarCell *cell in cellsToRemoveAfterAnimation) {
        CGRect frame = [cell frame];
        frame.origin.y -= yOffset;
        [cell setFrame:frame];
    }
...
{
    for (FlyingCalendarCell *cell in cellsToCleanup) {
...
- (FlyingCalendarCell *)_dequeueCell
{
    FlyingCalendarCell *cell = [[self spareCells] anyObject];
    
    if (!cell) {
        cell = [[FlyingCalendarCell alloc] initWithSize:[self _cellSize]];
    }
...
- (void)_moveCellFromSpareToUsed:(FlyingCalendarCell *)cell
{
...
- (void)_moveCellFromUsedToSpare:(FlyingCalendarCell *)cell
{
    //  Move the used cells to the appropriate set
...
- (void)setDataSource:(id<FlyingCalendarViewDataSource>)dataSource
{
    _dataSource = dataSource;
    [self reloadAnimated:NO];
}

#pragma mark - FlyingCalendarHeaderViewDataSource

- (NSString *)titleForHeader:(FlyingCalendarHeaderView *)header
{
    FlyingCalendarDisplayMode mode = [self displayMode];
    
    if(mode == FlyingCalendarViewModeMonth)
    {
        return [[self date] monthAndYearOnCalendar:[self calendar]];
    }
    
    else if (mode == FlyingCalendarViewModeWeek)
    {
...
- (BOOL)headerShouldHighlightTitle:(FlyingCalendarHeaderView *)header
{
    ...
    if (mode == FlyingCalendarViewModeDay) {
...
#pragma mark - FlyingCalendarHeaderViewDelegate
...
- (void)forwardTapped
{
    ...
    if ([self displayMode] == FlyingCalendarViewModeMonth) {
    ...
    else if([self displayMode] == FlyingCalendarViewModeWeek)
    {
...
- (NSUInteger)_rowCountForDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
    if (displayMode == FlyingCalendarViewModeWeek) {
        return 1;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FlyingCalendarTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    
    FlyingCalendarEvent *event = [[self events] objectAtIndex:[indexPath row]];
...
- (NSDate *)_firstVisibleDateForDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
...
- (NSDate *)_lastVisibleDateForDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
    // for the day mode, just return today
    if (displayMode == FlyingCalendarViewModeDay) {
        return [self date];
...
- (NSUInteger)_numberOfVisibleDaysforDisplayMode:(FlyingCalendarDisplayMode)displayMode
{
    ...
    else if (displayMode == FlyingCalendarViewModeMonth)
    {
        ...
        NSDate *firstVisible = [self _firstVisibleDateForDisplayMode:FlyingCalendarViewModeMonth];
        NSDate *lastVisible = [self _lastVisibleDateForDisplayMode:FlyingCalendarViewModeMonth];
