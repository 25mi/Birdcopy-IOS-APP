...
- (void) ensurePageLoaded
{
	...
	{
		fz_rect bounds;
		page = fz_load_page(ctx, doc, [@(number) intValue]);
	...
	if (!linkView) {
		dispatch_async(queue, ^{
			...
			dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self performSelector:@selector(hideLinks) withObject:nil afterDelay:5];
			});
		});
	}
}
...
- (void) clearSearchResults
{
    ...
    [self clearSpecialRecognize];
}

-(void) doeSpecialRecognize
{
    // Initialization code
...
-(void)clearSpecialRecognize
{
    [self removeGestureRecognizer:singleTapOne];
    
    [self removeGestureRecognizer:doubleTapOne];
}
...
- (NSString *) getAllText
{
    ...
    else
    {
    	dispatch_async(queue, ^{
            ...
            wholeText = [MuWord allText:words];
        });
...
-  (NSArray *) allWords
{
    ...
    else
    {
    	dispatch_async(queue, ^{
            [self ensurePageLoaded];
            wordsArray = enumerateWords(doc, page);
        });
...
-(NSString *) selectedText
{
    if (textSelectView) {
        ...
        return [textSelectView selectedText];
    }
    else
    {
        return nil;
...
- (void)processLongTap:(CGPoint)point;
{
    if(!wordsArray)
    {
        wordsArray = [self allWords];
    }

    point = [self  convertPoint:point toView:imageView];
    
    CGSize scale = fitPageToScreen(pageSize, self.bounds.size);
    
	point.x /= (scale.width*[self getScale]);
	point.y /= (scale.height*[self getScale]);

    
    BOOL refind=YES;
    if (currentWord)
    {
        if (CGRectContainsPoint(currentWord.rect, point) == true)
        {
            refind=NO;
        }
    }
    ...
    if (refind)
    {
        for (NSArray *line in wordsArray)
        {
            for (MuWord *word in line)
            {
                if (CGRectContainsPoint(word.rect, point) == true)
                {
                    ...
                    if(textSelectView)
                    {
                        [textSelectView redrawfor:currentWord];
                    }
...
{
    if (self.messagerDelegate&&
        [self.messagerDelegate respondsToSelector:@selector(handleSingleTap:)]) {
        
        [self.messagerDelegate handleSingleTap:recognizer];
    }
}
...
{
    if (self.messagerDelegate&&
        [self.messagerDelegate respondsToSelector:@selector(handleDoubleTap:)]) {
        
        [self.messagerDelegate handleDoubleTap:recognizer];
    }
}
